...
public class T4MSC : EditorWindow {
	...
	Texture[] LayerBump = new Texture[16];
	//Texture Layer1;
	//Texture Layer2;
	//Texture Layer3;
	//Texture Layer4;
	...
	void IniNewSelect()
	{
		...
		if (CurrentSelect && CurrentSelect.GetComponent <T4MObjSC>()  && CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial){
			...
			for (int i = 0; i < 16; ++i)
            {
				...
				else if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_BumpSplat" + myIndex))
                    LayerBump[i] = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_BumpSplat" + myIndex);
				else Layer[i] = null;
			}
